...
class RMSASBPPEnv(OpticalNetworkEnv):
    ...
    def _update_link_stats(self, node1: str, node2: str):
        ...
        if self.current_time > 0:
            ...
            if np.sum(slot_allocation) > 0:
                initial_indices, values, lengths = RMSASBPPEnv.rle(slot_allocation)
                ...
                if len(used_blocks) > 1:
                    ...
                    internal_idx, internal_values, internal_lengths = RMSASBPPEnv.rle(
    ...
    def get_available_blocks_working(self, path):  # Do we need to double that or use it two times ? (my thought: use it twice)
        ...
        # getting the blocks
        initial_indices, values, lengths = RMSASBPPEnv.rle(available_slots)
    ...
    def _get_network_compactness(self):
        ...
        for n1, n2 in self.topology.edges():
            ...
            initial_indices, values, lengths = \
                RMSASBPPEnv.rle(self.topology.graph['available_slots_working'][self.topology[n1][n2]['index'], :])
            used_blocks = [i for i, x in enumerate(values) if x == 0]
            if len(used_blocks) > 1:
                lambda_min = initial_indices[used_blocks[0]]
                lambda_max = initial_indices[used_blocks[-1]] + lengths[used_blocks[-1]]
...
def shortest_path_first_fit(env: RMSASBPPEnv) -> int:
...
def shortest_available_path_first_fit(env: RMSASBPPEnv) -> int:
...
def least_loaded_path_first_fit(env: RMSASBPPEnv) -> int:
    max_free_slots = 0
...
class SimpleMatrixObservation(gym.ObservationWrapper):
    ...
    def __init__(self, env: RMSASBPPEnv):
        super().__init__(env)
