...
from examples.graph_utils import read_sndlib_topology, read_txt_file, get_k_shortest_paths, get_path_weight, get_path_length, get_path_hops
    ...
    for idn1, n1 in enumerate(topology.nodes()):
        for idn2, n2 in enumerate(topology.nodes()):
            if idn1 < idn2:
                ...
                lengths = [get_path_length(topology, path) for path in paths]
                hops = [get_path_hops(topology, path) for path in paths]
                selected_modulations = [get_modulation_format(length, modulations) for length in lengths]
                objs = []
                for path, length, modulation, hop in zip(paths, lengths, selected_modulations, hops):
                    objs.append(Path(path_id=idp, node_list=path, length=length, hops = hop, best_modulation=modulation))
                    print(idp, length, modulation, path, hop)
                    idp += 1
...
topology = get_topology('/content/drive/MyDrive/optical-rl-gym-Modif/examples/topologies/eurocore.txt', 'eurocore', modulations, k_paths=k_paths)

with open(f'/content/drive/MyDrive/optical-rl-gym-Modif/examples/topologies/eurocore_eon_{k_paths}-paths.h5', 'wb') as f:
