...
def get_path_length(graph, path, length='length'):
    return np.sum([graph[path[i]][path[i+1]][length] for i in range(len(path) - 1)])

def get_path_hops(graph, path, length='length'):
    a = len(path) - 1
    return a
