        ...
        for idn2, n2 in enumerate(topology.nodes()):
            if idn1 < idn2:
                ...
                for path, length, modulation in zip(paths, lengths, selected_modulations):
                    objs.append(Path(path_id=idp, node_list=path, length=length, hops = 1, best_modulation=modulation))
...
topology = get_topology('/content/drive/MyDrive/optical-rl-gym-Modif/examples/topologies/six_topology.txt', 'six_topology', modulations, k_paths=k_paths)

with open(f'/content/drive/MyDrive/optical-rl-gym-Modif/examples/topologies/six_topology_eon_{k_paths}-paths.h5', 'wb') as f:
